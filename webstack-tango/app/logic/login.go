...
func (this *LoginLogic) CodeLogin(req *types.LoginReq) (*types.LoginResp, error) {
	if !this.CheckCode(req.Phone, req.Code, "login") {
...
func (this *LoginLogic) CheckCode(phone string, code string, scene string) bool {
	cacheCode := global.CACHE.Get("code:" + scene + ":" + phone)
	if cacheCode != code {
		return false
	}
	return true
...
func (this *LoginLogic) AddUser(req *types.RegisterReq) (uint64, error) {
	...
	if req.Password != "" {
		users.Salt = utils.RandString(4)
		users.Password = utils.CreatePassword(req.Password, users.Salt)
	}
...
func (this *LoginLogic) Register(req *types.RegisterReq) (*types.RegisterResp, error) {
	if !this.CheckCode(req.Phone, req.Code, "register") {
		return nil, errors.New("验证码错误")
	}
